...
def run_psql(database_url: str, ddl_path: Path) -> None:
    ...
    cmd = [psql, psql_url, "-f", str(ddl_path)]
    subprocess.run(cmd, check=True)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Apply db/ddl.sql to the configured PostgreSQL instance"
    )
    parser.add_argument(
        "--ddl", default="db/ddl.sql", help="Path to the DDL file (default: db/ddl.sql)"
    )
    parser.add_argument(
        "--database-url",
